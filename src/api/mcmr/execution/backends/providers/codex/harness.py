...
class CodexHarness(FrozenModel):
    ...
    def usage(self, stdout: str) -> tuple[int, int, int, int, str]:
        ...
        completed = self._completed_event(stdout)
        ...
        counts = [
            value if type(value := usage.get(name)) is int and value >= 0 else 0 for name in names
        ]
        stated_model = completed.get("model")
        model = stated_model.strip() if isinstance(stated_model, str) else ""
        return counts[0], counts[1], counts[2], counts[3], model
