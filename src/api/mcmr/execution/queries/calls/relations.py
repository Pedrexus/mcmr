...
from ....table import CallRelation
...
class CallCandidateRelations(CandidateRelations):
    """Build one contextual candidate for each resolved call site."""

    @classmethod
    def calls[Family: Fact](cls, table: Table[Family]) -> CandidateRelations:
        ...
        return cls(
            facts=facts,
            records=records,
            values=cls.value_rows(keyword_values, "keyword_names"),
        )
