...
from ....table import FunctionRelation
...
class FunctionCandidateRelations(CandidateRelations):
    ...
    @classmethod
    def function[Family: Fact](cls, table: Table[Family]) -> CandidateRelations:
    ...
    def function_facts[Family: Fact](cls, table: Table[Family]) -> pl.LazyFrame:
        ...
        return (
            functions.join(
                cls.lengths(controls, key="function_id", name="control_increments.length"),
                left_on="entity_id",
                right_on="function_id",
                how="left",
            )
            .join(
                cls.lengths(decorators, key="function_id", name="decorators.length"),
                left_on="entity_id",
                right_on="function_id",
                how="left",
            )
            .join(
                cls.lengths(evidence, key="function_id", name="evidence.length"),
                left_on="entity_id",
                right_on="function_id",
                how="left",
            )
            .join(
                cls.lengths(parameters, key="function_id", name="parameters.length"),
                left_on="entity_id",
                right_on="function_id",
                how="left",
            )
            ...
            .join(
                cls.lengths(references, key="function_id", name="references.length"),
                left_on="entity_id",
                right_on="function_id",
                how="left",
            )
            ...
            .select(
                ...
                *cls.span_columns(source="body_expression_", target="body_expression.span"),
                ...
                *cls.span_columns(source="definition_", target="definition.span"),
    ...
    def function_values[Family: Fact](cls, table: Table[Family]) -> pl.LazyFrame:
        ...
        return pl.concat(
            [
                cls.value_rows(decorator_values, "decorators"),
                cls.value_rows(tensor_values, "recognized_tensor_roles"),
            ],
            how="vertical",
        )
