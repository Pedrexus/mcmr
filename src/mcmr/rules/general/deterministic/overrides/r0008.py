...
@rule
def final_method_overridden(subject: OverrideFact) -> Count:
    ...
    """
    return sum(
        any(name.rsplit(".", 1)[-1] == "final" for name in inherited.decorators)
        for inherited, override in subject.overrides
        if inherited.parameters is not None and override.parameters is not None
    )
