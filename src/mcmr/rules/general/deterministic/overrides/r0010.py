...
@rule
def inherited_attribute_hides_a_method(subject: OverrideFact) -> Count:
    ...
    """
    return sum(
        inherited.parameters is None and override.parameters is not None
        for inherited, override in subject.overrides
        if not (inherited.name.startswith("__") and not inherited.name.endswith("__"))
    )
