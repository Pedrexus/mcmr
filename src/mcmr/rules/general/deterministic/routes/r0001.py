...
@rule
def duplicate_route_declaration(subject: RouteFact) -> Count:
    ...
    """
    declared = Counter(
        (route.method, route.path) for route in subject.routes if not route.is_prefix_composed
    )
    return sum(count > 1 for count in declared.values())
