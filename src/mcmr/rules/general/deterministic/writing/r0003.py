...
def sentence_length_uniformity(
    ...
    *,
    ...
    """
    eligible = [
        section.sentence_word_counts.at_least(minimum_words) for section in subject.sections
    ]
    return max(
        (counts.uniformity() for counts in eligible if len(counts) >= minimum_sentences),
        default=0.0,
    )
