...
def bounded_exception_region(
    ...
    region raises it rather than fighting it, and generated or vendored sources are excluded by
    provider selection instead. Widening the ceiling is only right when moving a statement out
    would change the protected transaction or the cleanup guarantee.
    ...
    """
    return max(
        (count for region in subject.regions for count in region.clause_statement_counts),
        default=0,
    )
