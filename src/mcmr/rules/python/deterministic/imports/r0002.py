...
def project_private_import(
    ...
    constant placement. External, unresolved, generated, and vendored imports are excluded, since
    none of them is a boundary this repository draws. A framework adapter that has to use a
    documented private API disables the rule at that one boundary rather than renaming somebody
    else's module.
    ...
    crosses_private_boundary = (
        subject.has_private_module_component
        or subject.is_private_member
        and not subject.is_private_uppercase_constant
    )
    return (
        subject.is_project_owned
        and crosses_private_boundary
