...
# A locator is free text except for grammar-reserved shapes.

# It stays on one line and cannot open with a space the preceding comma already wrote.
_LOCATORS = st.text(
...
@pytest.fixture(scope="module")
def report(definitions: Sequence[RuleDefinition]) -> InfluenceReport:
...
def test_every_reference_the_catalog_states_resolves_to_one_registered_source(
    report: InfluenceReport, definitions: Sequence[RuleDefinition]
) -> None:
...
def test_the_registry_fails_on_a_reference_naming_no_registered_work(
    definitions: Sequence[RuleDefinition], registry: WorkRegistry
) -> None:
    ...
    """
    thinned = WorkRegistry(works=[work for work in registry.works if work.title != "Refactoring"])
...
def test_the_table_names_the_work_rather_than_the_author(report: InfluenceReport) -> None:
    ...
    citations = [
        definition.id
        for definition, reference in report.index.references
        if reference.source == "A Philosophy of Software Design"
    ]

    assert books[0].source == "A Philosophy of Software Design"
    assert books[0].author == "John Ousterhout"
    assert books[0].references == len(citations)
    assert books[0].rules == len(set(citations))
...
def test_the_tool_half_is_present_and_told_apart_by_its_kind(report: InfluenceReport) -> None:
    ...
    assert sum(row.references for row in tools) == named
    assert {"Ruff", "Pylint", "Clippy"} <= {row.source for row in tools}
    assert [row.references for row in tools] == sorted(
        (row.references for row in tools), reverse=True
    )
...
def test_a_source_belonging_to_neither_registry_has_no_row(report: InfluenceReport) -> None:
    ...
    with pytest.raises(ValueError, match="neither a registered work nor a registered tool"):
        report.row("Unknown checker", ("ALL-ARCH0002",))
...
@given(data=st.data(), relation=st.sampled_from(Relation), locator=_LOCATORS)
