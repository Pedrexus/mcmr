...
_COMPLEXITY = ClangTidyMagnitudeOracle(
    rules=("readability-function-cognitive-complexity",),
    settings={"readability-function-cognitive-complexity.Threshold": "0"},
)

# Disable two heuristics before this check answers MCMR's question. MCMR requires identically
# written types and does not infer safety from dissimilar parameter names.

# These are configuration differences rather than disagreements about swappable parameters.
_SWAPPABLE = ClangTidyOracle(
...
_UNUSED_RESULT = ClangTidyOracle(rules=("bugprone-unused-return-value",))
...
def indented(depth: int) -> str:
    ...
    opening = [
        f"{_STEP * (index + 1)}{blocks[index % len(blocks)].format(index)}"
    ...
    return "\n".join(
        [
            ...
            f"{_STEP}total = 0",
            *opening,
            f"{_STEP * (depth + 1)}total += 1",
            f"{_STEP}return total",
            "",
        ]
    )
...
def test_cognitive_complexity_agrees_with_clang_tidy_and_the_python_twin(
    ...
    `ALL-FUNC0008` scores a structure at one plus the number of structures enclosing it, which is
    ...
    general rule reads.
    """
    root = trees.grow({_UNIT: braced(depth), _TWIN: indented(depth)})
    scored = depth * (depth + 1) // 2
    rule = DeclarationReader(rule_id="ALL-FUNC0008", family=FunctionFact)
    stated = Report(
        reader="the strategy",
        sites=[Site.at(_UNIT, 1) for _ in range(scored)],
    )

    oracle = _COMPLEXITY.report(root)
    differ(
        stated,
        Relation.EQUALS,
        oracle,
    ...
    assert (
        ...
        .report(root)
        .states(*(Site(path=_TWIN, line=1, through=depth + 4) for _ in range(scored)))
    )
    differ(
...
def test_swappable_parameters_agrees_with_clang_tidy_on_one_pair(
    ...
    root = written(
        tmp_path,
        {
            _UNIT: """int pairwise(int a, int b) {
  int total = 0;
  total += a;
  total += b;
  return total;
...
"""
    ...
    oracle = _SWAPPABLE.report(root)

    assert oracle.states(Site.at(_UNIT, 1))
    differ(
...
def test_swappable_parameters_counts_pairs_where_clang_tidy_reports_a_run(
    ...
    root = written(
        tmp_path,
        {
            _UNIT: """int score(int a, int b, int limit) {
  int total = 0;
  total += a;
  total += b;
  total += limit;
...
"""
    ...
    oracle = _UNUSED_RESULT.report(root)

    assert oracle.states(Site.at(_UNIT, 3))
    differ(
        DeclarationReader(
