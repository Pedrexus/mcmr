...
# One crate states every lifetime arrangement so both readers answer about the same source. Line
# numbers matter because the comparison checks which declaration each reader identifies.
...
def crate(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    return written(tmp_path_factory.mktemp("elision"), {"Cargo.toml": _MANIFEST, _LIBRARY: _CRATE})
...
def test_every_annotation_mcmr_calls_idle_is_one_clippy_calls_idle(crate: Path) -> None:
    ...
    assert oracle.states(*(Site.at(_LIBRARY, line) for line in (13, 25, 30)))
    assert ours.states(*(Site.at(_LIBRARY, line) for line in (13, 30)))
    differ(
        ours,
        Relation.SUBSET,
        oracle,
...
def test_the_one_arrangement_mcmr_declines_is_the_one_it_documents(crate: Path) -> None:
    ...
    differ(
        ours,
        Relation.EQUALS,
        oracle.minus(Site.at(_LIBRARY, 25)),
...
def test_statement_without_effect_agrees_with_clippy(tmp_path: Path) -> None:
    ...
    root = written(
        tmp_path,
        {
            ...
            _LIBRARY: "pub fn check(value: i32) -> i32 {\n    value == 3;\n    value\n}\n",
        },
    )
    oracle = Oracle.of("clippy", "no_effect").report(root)

    assert oracle.states(Site.at(_LIBRARY, 2))
    differ(
        DeclarationReader(
