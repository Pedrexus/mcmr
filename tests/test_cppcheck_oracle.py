...
# MCMR states no reference to Cppcheck anywhere in the catalog, so nothing here is a claim being
# checked. These are the two questions both readers already answer about the same C++, measured so
# that a claim, if one is ever written, starts from a number somebody took rather than from a hope.
CONST_STATEMENT = "constStatement"
UNUSED_FUNCTION = "unusedFunction"
...
def test_statement_without_effect_agrees_with_cppcheck(tmp_path: Path) -> None:
    ...
    oracle = Oracle.of("cppcheck", CONST_STATEMENT).report(root)
...
def test_unreferenced_declaration_agrees_with_cppcheck_on_what_nothing_reaches(
    ...
    oracle = Oracle.of("cppcheck", UNUSED_FUNCTION).report(root)
