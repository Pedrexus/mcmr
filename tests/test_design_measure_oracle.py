...
PUBLIC_METHODS = PylintMagnitudeOracle(
    rules=("too-many-public-methods",), options=("--max-public-methods=0",)
)
INSTANCE_ATTRIBUTES = PylintMagnitudeOracle(
    rules=("too-many-instance-attributes",), options=("--max-attributes=0",)
)
ANCESTORS = PylintMagnitudeOracle(rules=("too-many-ancestors",), options=("--max-parents=0",))

SURFACE = MeasureReader(rule_id="ALL-CLAS0007", family=ClassFact)
FIELDS = MeasureReader(rule_id="ALL-CLAS0008", family=SymbolReachFact)
ANCESTRY = MeasureReader(rule_id="ALL-CLAS0009", family=OverrideFact)
...
def measured(sites: dict[str, tuple[int, int]]) -> Report:
...
def test_public_method_count_agrees_with_pylint(
    trees: Trees, drawn: dict[str, tuple[int, int]]
) -> None:
    ...
    root = trees.grow(
        {
            f"{name}.py": class_with_methods("Surface", public, hidden)
            for name, (public, hidden) in drawn.items()
        }
    )
    oracle = PUBLIC_METHODS.report(root)
...
def test_declared_field_count_agrees_with_pylint_on_receiver_state(
    ...
    oracle = INSTANCE_ATTRIBUTES.report(root)
