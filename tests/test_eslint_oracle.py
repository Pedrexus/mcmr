...
def test_nesting_depth_is_the_ceiling_eslint_falls_silent_at(trees: Trees, depth: int) -> None:
    ...
    exactly one and neither locates its finding in the same place: ESLint names the block and
    `FunctionFact` names the declaration.
    ...
    write three.
    """
    root = trees.grow({MODULE: nested(depth)})
    ours = DeclarationReader(rule_id="ALL-FUNC0013", family=FunctionFact, suffixes=(".ts",))

    assert ours.report(root).states(*(Site.at(MODULE, 1) for _ in range(depth - 1)))
    assert ESLintOracle(rules=("max-depth",), ceiling=depth).report(root).states()
    assert (
        ESLintOracle(rules=("max-depth",), ceiling=depth - 1)
        .report(root)
        .states(Site.at(MODULE, depth + 1))
    )
...
def test_required_parameter_count_agrees_with_typescript_eslint(trees: Trees, width: int) -> None:
    ...
    root = trees.grow(
        {MODULE: f"export function take({parameters}): number {{\n  return 1;\n}}\n"}
    )
    oracle = ESLintMagnitudeOracle(rules=("@typescript-eslint/max-params",), ceiling=0)

    differ(
        DeclarationReader(rule_id="ALL-FUNC0014", family=FunctionFact, suffixes=(".ts",)).report(
            root
        ),
        Relation.EQUALS,
        oracle.report(root),
...
def test_escape_hatch_density_covers_two_typescript_eslint_rules_and_the_assertion(
    ...
    root = written(
        tmp_path,
        {
            MODULE: "export function widen(value: unknown): string {\n"
    ...
    assert oracle.states(Site.at(MODULE, 2), Site.at(MODULE, 3))
    differ(
        FindingReader(rule_id="TS-TYPE0002", family=ModuleSurfaceFact, suffixes=(".ts",)).report(
            root
        ),
        Relation.EQUALS,
        oracle.plus(Site.at(MODULE, 2)),
...
def test_the_console_and_debugger_claims_equal_the_eslint_union(
    ...
    root = written(
        tmp_path,
        {
            MODULE: "export function run(value: number): number {\n"
            "  console.log(value);\n"
            "  debugger;\n"
            "  return value;\n"
            "}\n",
    ...
    assert console.states(Site.at(MODULE, 2))
    assert debugger.states(Site.at(MODULE, 3))
...
def test_the_unused_expression_claim_agrees_with_eslint(
    ...
    root = written(
        tmp_path,
        {
            MODULE: "export function run(order: {total: number}): number {\n"
            "  order.total;\n"
            "  order.total === 3;\n"
            "  return order.total;\n"
            "}\n",
