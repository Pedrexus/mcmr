...
from mcmr.cli import imports, proposed
from mcmr.cli import simulate as simulate_command
from mcmr.projections import Dependency, JsonRendering, ModuleGraph
from mcmr.simulation import (
    ImportProposal,
    ProposedImport,
    SimulationFormat,
    SimulationText,
...
needs_archy = pytest.mark.skipif(
    ...
    reason="the simulation oracle needs the Archy checkout and uv",
)
...
class Oracle(BaseModel):
    """What Archy predicts for one import delta, in the fields MCMR states too.

    Archy also reports a composite score and layer rules, and MCMR has neither, so only the three
    answers both tools define the same way over the same graph are read out here.
    """
    ...
    @classmethod
    def ask(cls, root: Path, *arguments: str) -> Oracle:
        """Return what Archy predicts for the same import delta over the same tree."""
        ...
        return cls(
            cycles=tuple(tuple(item["modules"]) for item in report["cycles"]["added"]),
            back_edges=tuple(
                (item["source"], item["target"]) for item in report["new_back_edges"]
            ),
            before=report["propagation_cost"]["before"],
            after=report["propagation_cost"]["after"],
        )
...
def test_the_simulation_text_names_every_section_and_bounds_each_one() -> None:
    ...
    assert "2 imports added and 1 removed, in the graph alone, with no file touched" in rendered
    assert "propagation cost 0.3600 becomes " in rendered
    assert "Added (2)\n  pkg.store imports pkg.cli\n  and 1 more" in rendered
    assert "Removed (1)\n  pkg.cli imports pkg.util" in rendered
    assert "Already as asked (1)\n  pkg.cli imports pkg.engine" in rendered
    assert "Stated both ways (0)" in rendered
    assert "Unknown modules (1)\n  pkg.gone" in rendered
    assert "Cycles formed (1)" in rendered
    assert "Cycles broken (0)" in rendered
    assert "Back edges formed (1)" in rendered
    assert "Back edges cleared (0)" in rendered
...
def test_the_simulate_command_answers_over_a_real_repository(
    ...
    simulate_command(root, add="pkg.store:pkg.cli", kernel=BINARY)
    assert "Cycles formed (1)\n  pkg.cli pkg.engine pkg.store" in capsys.readouterr().out

    simulate_command(
        root, add="pkg.store:pkg.cli", format=SimulationFormat.JSON, kernel=BINARY, exclude=""
    )
...
def test_the_simulation_agrees_with_the_archy_oracle_on_the_same_edge(tmp_path: Path) -> None:
    ...
    theirs = Oracle.ask(root, "--add", "pkg.store:pkg.cli")

    assert [tuple(sorted(cycle.members)) for cycle in ours.cycles_formed] == [
        tuple(sorted(cycle)) for cycle in theirs.cycles
    ]
...
def test_the_simulation_agrees_with_the_archy_oracle_on_removing_an_edge(tmp_path: Path) -> None:
    ...
    theirs = Oracle.ask(root, "--remove", "pkg.engine:pkg.store")
